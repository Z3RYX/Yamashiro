...
namespace Akari.Net.REST
{
    internal static class CDNEndpoints
    {
        ...
        private const string BaseAddress = "https://cdn.discordapp.com/";
        ...
        internal static string GetCustomEmojiEndpoint(ISnowFlake EMOJI_ID ,CDNExtension EXT)
        {
            ...
            return BaseAddress + CustomEmoji.Replace("%EMOJI_ID%", EMOJI_ID.ToString()).Replace("%EXT%", ext);
        }

        internal static string GetGuildIconEndpoint(ISnowFlake GUILD_ID, string GUILD_ICON, bool ANIMATED, CDNExtension EXT)
        {
            string ext = GetExtension(EXT);
            return BaseAddress + GuildIcon.Replace("%GUILD_ID%", GUILD_ID.ToString()).Replace("%ANI%", ANIMATED ? "a_" : "").Replace("%GUILD_ICON%", GUILD_ICON).Replace("%EXT%", ext);
        }

        internal static string GetGuildSplashEndpoint(ISnowFlake GUILD_ID, string GUILD_SPLASH, CDNExtension EXT)
        {
            string ext = GetExtension(EXT);
            return BaseAddress + GuildSplash.Replace("%GUILD_ID%", GUILD_ID.ToString()).Replace("%GUILD_SPLASH%", GUILD_SPLASH).Replace("%EXT%", ext);
        }
        ...
        {
            string ext = GetExtension(EXT);
            return BaseAddress + GuildDiscoverySplash.Replace("%GUILD_ID%", GUILD_ID.ToString()).Replace("%GUILD_DISCOVERY_SPLASH%", GUILD_DISCOVERY_SPLASH).Replace("%EXT%", ext);
        }

        internal static string GetGuildBannerEndpoint(ISnowFlake GUILD_ID, string GUILD_BANNER, CDNExtension EXT)
        {
            string ext = GetExtension(EXT);
            return BaseAddress + GuildBanner.Replace("%GUILD_ID%", GUILD_ID.ToString()).Replace("%GUILD_BANNER%", GUILD_BANNER).Replace("%EXT%", ext);
        }
        ...
        internal static string GetDefaultUserAvatarEndpoint(int DISCRIMINATOR, CDNExtension EXT)
        {
            ...
            string ext = GetExtension(EXT);
            return BaseAddress + DefaultUserAvatar.Replace("%DISCRIM%", discrim.ToString()).Replace("%EXT%", ext);
        }

        internal static string GetUserAvatarEndpoint(ISnowFlake USER_ID, string USER_AVATAR, bool ANIMATED, CDNExtension EXT)
        {
            string ext = GetExtension(EXT);
            return BaseAddress + UserAvatar.Replace("%USER_ID%", USER_ID.ToString()).Replace("%ANI%", ANIMATED ? "a_" : "").Replace("%USER_AVATAR%", USER_AVATAR).Replace("%EXT%", ext);
        }

        internal static string GetApplicationIconEndpoint(ISnowFlake APP_ID, string APP_ICON, CDNExtension EXT)
        {
            string ext = GetExtension(EXT);
            return BaseAddress + ApplicationIcon.Replace("%APP_ID%", APP_ID.ToString()).Replace("%APP_ICON%", APP_ICON).Replace("%EXT%", ext);
        }

        internal static string GetApplicationAssetEndpoint(ISnowFlake APP_ID, string ASSET_ID, CDNExtension EXT)
        {
            string ext = GetExtension(EXT);
            return BaseAddress + ApplicationAsset.Replace("%APP_ID%", APP_ID.ToString()).Replace("%ASSET_ID%", ASSET_ID).Replace("%EXT%", ext);
        }

        internal static string GetTeamIconEndpoint(ISnowFlake TEAM_ID, string TEAM_ICON, CDNExtension EXT)
        {
            string ext = GetExtension(EXT);
            return BaseAddress + TeamIcon.Replace("%TEAM_ID%", TEAM_ID.ToString()).Replace("%TEAM_ICON%", TEAM_ICON).Replace("%EXT%", ext);
        }
